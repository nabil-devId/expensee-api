...
async def calculate_ocr_completion_time(image_bytes: bytes) -> int:
    ...
    """
    try:
        # Base processing time (seconds) - reduced from 5 to 1 based on real-world observations
        base_time = 2
        ...
        try:
            ...
            return 3  # Default estimate for unidentifiable images (reduced from 15)
        except Exception as img_error:
            logger.error(f"Error loading image: {str(img_error)}")
            return 2  # Fallback estimate (reduced from 10)
        
        # Factor 1: Image size - reduced impact
        # Larger images take longer to process, but not as much as previously estimated
        try:
            size_factor = (width * height) / (2000 * 2000)  # Normalized to 4MP
            size_factor = min(max(size_factor, 0.5), 1.5)  # Reduced upper limit from 5 to 1.5
        except Exception as size_error:
            logger.error(f"Error calculating size factor: {str(size_error)}")
            size_factor = 1.0  # Default size factor
        
        # Factor 2: Image complexity - reduced impact
        try:
            ...
            if sum(histogram) == 0:
                ...
                # Calculate histogram variance as a simplistic measure of complexity
                ...
                complexity_factor = min(max(variance / 2000, 0.8), 1.5)  # Reduced upper limit from 2.5 to 1.5
        except Exception as complexity_error:
            logger.error(f"Error calculating complexity factor: {str(complexity_error)}")
            complexity_factor = 1.0  # Default complexity factor
        
        # Factor 3: Historical factor - reduced from 1.2 to 1.0 based on performance data
        historical_factor = 1.0
        
        # Calculate final estimate
        estimate = base_time * size_factor * complexity_factor * historical_factor
        
        # Add a smaller random variance (±5%) to make it seem more realistic
        import random
        variance = random.uniform(0.95, 1.05)  # Reduced from ±10% to ±5%
        estimate *= variance
        
        # Calculate final result, with a minimum of 1 second and maximum of 5 seconds
        # This cap ensures we never severely overestimate based on real-world observations
        result = max(min(int(math.ceil(estimate)), 5), 1)
        
        # Log the details of the calculation for debugging
        ...
        # Default fallback estimate for any unexpected errors - reduced from 12 to 3
        return 3
...
async def process_receipt_with_textract(image_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 6. Post-process and validate extracted data
...
def _calculate_confidence_scores(textract_response: Dict[str, Any]) -> Dict[str, float]:
    ...
    Returns:
        Dict with field names and confidence scores
    """
    confidence_scores = {}
    
    # Extract confidence scores for summary fields
...
def _validate_amount_str(amount_str: str) -> str:
    ...
    if ',' in amount_str and '.' in amount_str:
        ...
        if len(parts) > 1 and len(parts[-1]) <= 2:
            # Likely a decimal separator (e.g., 1234,56)
            amount_str = amount_str.replace(',', '.')
        else:
...
def _extract_payment_method(textract_response: Dict[str, Any]) -> Optional[str]:
    """Extract payment method from Textract response"""
    for expense_document in textract_response.get('ExpenseDocuments', []):
        for expense_field in expense_document.get('SummaryFields', []):
...
def _validate_merchant_name(merchant_name: Optional[str]) -> str:
    """Validate and normalize merchant name"""
...
def _validate_payment_method(payment_method: Optional[str]) -> str:
    ...
    # Normalize common payment methods
...
def _validate_line_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        # Ensure total = price * quantity if total is zero
